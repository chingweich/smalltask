...
void plot_Asymptotic_altascombine2(string outputdir, string mode)
{
...
for(int i=0;i<8;i++){
  ...
  TFile* Fgz08=TFile::Open("gz08.root");
  TFile* Fgzfo=TFile::Open("gzfo.root");
  TGraph* gz08=(TGraph*)Fgz08->FindObjectAny("Graph");
  TGraph* gzfo=(TGraph*)Fgzfo->FindObjectAny("Graph");
  
  
  gz08->SetLineColor(kBlue);
  gz08->SetLineWidth(3);
  //grthSM->SetLineStyle(1);
  gz08->SetLineStyle(kDashed);
  gz08->SetFillColor(kBlue);
  gz08->SetFillStyle(3344);

  gzfo->SetLineColor(kRed);
  gzfo->SetLineWidth(3);
  //grthSM10->SetLineStyle(1);
  gzfo->SetLineStyle(kDashed);
  gzfo->SetFillColor(kRed);
  gzfo->SetFillStyle(3344);
      ...
      for(size_t b_i = 0; b_i < b_up; ++b_i)
        {
          ...
          gz08->GetPoint(a_i, x1, y1);
          gz08->GetPoint(a_i+1, x2, y2);
