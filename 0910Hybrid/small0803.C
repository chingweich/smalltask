...
TH2D* small0706Base(string inputDir,string outputName,int option=0,int retrunexp=0){
	...
	for(int i=0;i<8;i++){
		for(int j=0;j<6;j++){
				...
				TFile* tf1;
				TTree* tree;
				tf1=TFile::Open(Form("%sMZp%dGeV_MA0_%dGeV/higgsCombineTest_Merged.root",inputDir.data(),massZ[i],massA[j]));
				if(!tf1)continue;
				//TDirectory * dir;
...
void small0803(){
	...
	TH2D* th2,*th3,*th4;
	th2=small0706Base("CombinedMonoHDatacard_HybridNew/MonoHCombo","limit_combination",0,2);
	th3=small0706Base("CombinedMonoHDatacard_HybridNew/MonoHCombo","limit_combination",0,3);
